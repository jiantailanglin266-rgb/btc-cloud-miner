...
### 2026-08-17 精密化v3 の実 API 再検証

| 項目 | 実測値 | 判定 |
|---|---|---|
| ブロック手数料 | **0.01478 BTC/block（MEASURED_BLOCKS）** — 直近ブロック実測トリム平均が proxy を置換 | ✓ |
| 難易度調整予測 | 実 API から −0.68% を取得。リターゲットは注文期間外 → retargetWeight=0・現在難易度を使用（推測で埋めない動作） | ✓ |
| 板の総供給量（単位監査） | 183,600 TH/s ≪ 網 914.7M TH/s → 単位整合 PASS | ✓ |
| 判定 | 実勢 0.5199 vs break-even 0.3908 → margin −24.8% で WAIT 継続 | ✓ |
